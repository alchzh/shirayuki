...
export async function confirm(message, prompt, force) {
  ...
  const confirmMessage = await message.channel.send(
    `${prompt} Confirm by reacting with :thumbsup: or typing \`yes\`.`
  );
...
export async function executeCommand({ message, command: commandStr, arguments: _args }) {
  const command = findCommand(message.client, commandStr);
  ...
  try {
    ...
    if (command.minRooms !== undefined) {
      ...
      if (message.mentions.rooms.size < command.minRooms) {
        throw new ArgumentsError(
          `invalid number of rooms mentioned. ${command.minRooms} teams expected, ${message.mentions.rooms.size} found.`
        );
      }
    }

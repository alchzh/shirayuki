...
export function test(conditions) {
  return function _test(obj, _this) {
    ...
    if (conditions.parent) {
      const parent = _this.resolve(conditions.parent);
...
BaseManager.prototype.resolveRegistered = function resolveRegistered(propName) {
  ...
  if (existing && !existing.deleted && predicate(existing, this)) return existing;

  const newObj = this.cache.find(s => predicate(s, this));
  if (newObj) {
...
BaseManager.prototype.resolveRegisterName = function resolveRegisterName(resolvable) {
  ...
  return this.register.findKey(predicate => predicate(obj, this));
};

...
export function isDefaultRoom(room) {
  ...
  return (
    room.id === room.guild.channels.controlRoomCategory.id ||
    room.id === room.guild.channels.hubCategory.id ||
    room.id === room.guild.channels.honorPledgeCategory.id ||
...
export async function createRoom(guild, name, staffSpectatorInvisible = false) {
  ...
  if (!staffSpectatorInvisible) {
    await room.updateOverwrites([guild.roles.staff, guild.roles.spectator], {
      VIEW_CHANNEL: true,
    });
  }
